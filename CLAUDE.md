...
- **Two group kinds** (`Group.kind`): `manual` (user `+` button — default if field missing) and `auto-domain` (created by autoGroupByDomain). Auto-domain groups have an `autoDomain` field used to merge later auto-group passes; renaming an auto-domain group keeps its merge identity. **Auto-domain groups self-delete when emptied** (call `cleanupEmptyAutoGroups` after any mutation that removes from `group.tabs`); manual groups stay even when empty.
